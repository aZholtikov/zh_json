...
static void s_zh_json_resize(zh_json_t *json, uint8_t capacity);
static void *s_zh_json_get_name(zh_json_t *json, uint8_t index);
static void *s_zh_json_get_value(zh_json_t *json, uint8_t index);
...
void zh_json_create(zh_json_t *json, char *buffer)
{
    ...
    {
        strcat(buffer, "\"");
        strcat(buffer, s_zh_json_get_name(json, i));
        strcat(buffer, "\": \"");
        strcat(buffer, s_zh_json_get_value(json, i));
        strcat(buffer, "\"");
        if (i != json->size - 1)
        {
            strcat(buffer, ", ");
